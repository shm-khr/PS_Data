...
vec VEC(int n){vec a;rep(i,n)a.pb(in());return a;}
const ll MOD=1e9+7;
const int N=1100000;
...
vecll ans;
int st;
int dfs(int x,int p,vector<ll> &v){
    vecll mem;
    int res=0;
    for(auto e:g[x]){
        if(e==p)continue;
        int d = dfs(e,x,v);
        ans[c[x]]+=(ll)(d-v[c[x]])*(d-v[c[x]]+1)/2;
        if(x==st){
            rep(i,c.size()){
                if(i!=c[x]){
                    ans[i]+=(ll)(d+1-v[i])*(d+2-v[i])/2;
                }
            }
        }
        res+=d;
