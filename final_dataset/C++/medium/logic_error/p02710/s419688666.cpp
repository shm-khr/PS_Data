...
void dfs(ll cpvs[200005], M<ll, ll> &ctopv,
         const V<ll> children[200005], const V<ll> clist, ll v)
{
    ll c = clist[v];

    if (children[v].empty()) {
        if (ctopv.find(c) == ctopv.end()) ctopv[c] = 1;
        cpvs[v] = ctopv[c];
        return;
    }
    ...
    for (auto cv : children[v]) {
        ...
        dfs(cpvs, ctopv, children, clist, cv);
...
{
    ll n; llin(n);
    S V<ll> clist; clist.pb(0LL); llina(clist, n);
    S V<t2> uvlist; llinl2(uvlist, n-1);

    uvlist.pb(t2(0, 1));

    S V<ll> edgelist[200005];
    make_edgelist(uvlist, edgelist);

    S ll parents[200005];
    S V<ll> children[200005];
    S ll levels[200005];
    make_parental_relation(edgelist, 0, n+1, parents, children, levels);
    ...
    S ll cpvs[200005] = {};
    S M<ll, ll> ctopv;
    dfs(cpvs, ctopv, children, clist, 1);

    debug_printf("---- cpvs\n");
    debug_print(cpvs, n+1);

    S ll partszs[200005];
    srep (v, 1, n+1) partszs[v] = vcnts[v];
    S ll topszs[200005];
    srep (c, 1, n+1) topszs[c] = n;

    srep (v, 1, n+1) {
        ll c = clist[v];
        ll pv = cpvs[v];
        if (pv == 1) topszs[c] -= vcnts[v];
        else partszs[pv] -= vcnts[v];
    }

    debug_printf("---- partszs\n");
    debug_print(partszs, n+1);
    debug_printf("---- topszs\n");
    debug_print(topszs, n+1);

    S V<ll> colorparts[200005];
    srep (v, 2, n+1) {
        ll c = clist[parents[v]];
        if (partszs[v]) colorparts[c].pb(partszs[v]);
    }
    srep (c, 1, n+1) {
        if (topszs[c]) colorparts[c].pb(topszs[c]);
    }

    srep (c, 1, n+1) {
        debug_printf("---- colorparts %lld\n", c);
        debug_print(colorparts[c]);
    }

    ll all = n;
    if (all >= 2LL) all += combination(n, 2LL);

    debug_printf("---- ans\n");
    srep (c, 1, n+1) {
        ll ans = all;
        for (auto sz : colorparts[c]) {
            ll decr = sz;
            if (sz >= 2LL) decr += combination(sz, 2LL);
            ans -= decr;
        }
        cout << ans << endl;
    }
