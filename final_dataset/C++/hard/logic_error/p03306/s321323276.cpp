...
template<typename T> inline T limit(T val, T low, T high) { return val < low ? low : val > high ? high : val; }
...
void dfs(int idx = 0, ll accum = 0, int flip = 1) {

    if (flip == -1)
        minset(ans_high, accum);
    else
        maxset(ans_low, -accum);

    if (-1 * visited[idx] == flip) {
        // odd_cycle

        // t <= visited[idx] * dp[idx] && flip * accum <= t
        ll t = visited[idx] * dp[idx] + flip * accum;
        if (t % 2) bye("0");
        ans_low = ans_high = t;
...
int main() {
    ...
    cout << max(0ll, ans_high - ans_low + 1) << endl;
