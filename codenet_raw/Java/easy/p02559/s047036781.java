...
public class Main {
    public static void main(final String[] args) {
        ...
        final SegmentTree<Integer> segmentTree = new SegmentTree<>(list, 0, Integer::sum);
        IntStream.range(0, q)
            .mapToObj(i -> new Query(scanner.nextInt(), scanner.nextInt(), scanner.nextInt()))
            .forEach(query -> query.exec(segmentTree));
    }
    ...
    static class Query {
        ...
        void exec(final SegmentTree<Integer> segmentTree) {
            if (t == 0) {
                segmentTree.update(q1, v -> v + q2);
            } else {
                System.out.println(segmentTree.query(q1, q2));
            }
        }
    }
