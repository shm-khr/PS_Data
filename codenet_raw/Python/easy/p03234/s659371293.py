...
def dp(i, j):
    ...
    if gg == 0:
        return 0
    # return gg-sum([dp(i, k)*g(k+1, j) for k in range(i+1, j)]) % cut
    return (gg-sum([dp(i, k)*ggg[k+1][j] for k in nonSandwichedNums[i] if k < j])) % cut


print(sum([dp(i, j)*pairCombinations(undetermined-numberUnderterminedBetween(i, j)) for i, j in itertools.combinations(range(1, n+1), 2)]) % cut)
