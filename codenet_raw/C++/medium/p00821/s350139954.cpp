	...
	while (1) {
		...
		for (int i = -SIZE; i < SIZE; ++i) {
			for (int j = -SIZE; j < SIZE; ++j) {
				vector<Point>pas;
				pas.emplace_back(i + 1e-4, j + 1e-4);
				pas.emplace_back(i + 1 - 1e-4, j + 1e-4);
				pas.emplace_back(i + 1 - 1e-4, j + 1 - 1e-4);
				pas.emplace_back(i + 1e-4, j + 1 - 1e-4);
				for (int k= 0; k< 4; ++k) {
					if (is_in_polygon(ps, pas[k])) {
						oks[j + SIZE][i + SIZE] = true;
					}
				}
			}
		}
		for (int i = 0; i < M; ++i) {
			Line l(ps[i], ps[(i + 1) % M]);
			if (l[0].real()>l[1].real()) {
				Point p = l[0];
				l[0] = l[1];
				l[1] = p;
			}
			if (abs(l[0].real()-l[1].real())<eps)continue;
			if (abs(l[0].imag() - l[1].imag()) < eps)continue;
			for (int left= l[0].real()+eps; left < l[1].real()-eps; ++left) {
				const ld ly = (l[0].imag()*(l[1].real() - left) + l[1].imag()*(left - l[0].real())) / (l[1].real() - l[0].real());
				const int right = left + 1; 
				const ld ry = (l[0].imag()*(l[1].real() - right) + l[1].imag()*(right - l[0].real())) / (l[1].real() - l[0].real());
				int ily,iry;
				if (ly < ry) {
					ily = int(ly + 1e-5);
					iry = int(ry - 1e-5);
					if (ly < 0)ily--;
					if (ry < 0)iry--;
					for (int y = ily; y <= iry; ++y) {
						oks[y + SIZE][left + SIZE] = true;
					}
				}
				else {
					ily = int(ly - 1e-5);
