...
long long det(mat A){
	...
	for(int i=0;i<n;i++){
		...
		for(map<int,long long>::iterator it=A[i].begin();it!=A[i].end();it++){
			A[i][(*it).first]=A[i][(*it).first]*t%mod;
		}
		for(int j=i+1;j<n;j++){
			if(A[j].count(i)&&A[j][i]){
				long long div=A[j][i];
				for(map<int,long long>::iterator it=A[i].begin();it!=A[i].end();it++){
					...
					if(!A[j].count((*it).first))A[j][(*it).first]=mod-(*it).second*div%mod;
					else A[j][(*it).first]=(A[j][(*it).first]+mod-(*it).second*div%mod)%mod;
				}
			}
		}
		/*printf("%lld\n",val);
		for(int j=0;j<n;j++){
			for(int k=0;k<n;k++){
				if(!A[j].count(k))printf("0 ");
				else printf("%lld ",A[j][k]);
			}
			printf("\n");
		}*/
	}
