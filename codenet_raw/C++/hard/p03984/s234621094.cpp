...
void solve() {
	...
	rhr.init(hr);
	int sym=0;
	FOR(i,N) if(rhv.hash(i,i+N-1)==rhr.hash(N-1-i,N-1-i+N-1)) sym=1;
	if(sym==0 && N%2==0) {
		FOR(i,N) if(rhv.hash(i,i+N-1)==rhr.hash(N-i,N-i+N-1)) sym=2;
	}
	...
	if(sym==0) {
		...
		cout<<ret%mo*modpow(N/minper)%mo<<endl;
	}
	else if(sym==1) {
		assert(0);
	}
	...
	/*
	FOR(i,N) {
		// vertex is on flip line
		x=y=N+i;
		FOR(j,N) if(RH[(x+j)%N]!=RH[(y-j)%N]) break;
		if(j==N) {
			sym=1;
			for(i=1;i<=L.size()/minper;i++) if(phi[i]) {
				
				ll DDn=1,DDs=1;
				// normal
				FOR(j,i*minper) DDn=DDn*RD[j]%mo;
				// sym
				FOR(j,(i*minper+2)/2) DDs=DDs*RD[j]%mo;
				//_P("%d %d %lld %lld\n",i,phi[i],DDn,DDs);
				ret += (DDn + DDs)*phi[i]%mo;
			}
			//_P("%lld / %d = %lld\n",ret,2*L.size()/minper,ret%mo*modpow(2*L.size()/minper,mo-2)%mo);
			return _P("%lld\n",ret%mo*modpow(2*L.size()/minper)%mo);
		}
		// vertex is not on flip line (only if N is even)
		x=i;
		y=i+N-1;
		FOR(j,N) if(RH[(x+j)%N]!=RH[(y-j)%N]) break;
		if(j==N) {
