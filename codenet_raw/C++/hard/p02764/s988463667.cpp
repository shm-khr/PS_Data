...
struct epsdou {
    ...
    template<class T> bool operator<=(T b) { return v < (double) b || fabs(v - b) <= eps; }
    template<class T> bool operator>=(T b) { return v > (double) b || fabs(v - b) <= eps; }
...
template<class T> string out_m2(vector<vector<T> > &a, ll H = inf, ll W = inf, int key = -1) {H = (H!=inf)? H : min({H, sz(a), 12ll});W = min({W, sz(a[0]), 12ll});stringstream ss;ss << endl;if (key == -1)ss << " *|"; else ss << " " << key << "|";rep(w, W)ss << std::right << std::setw(4) << w;ss << "" << endl;rep(w, W * 4 + 3)ss << "_";ss << "" << endl;rep(h, H) {ss << std::right << std::setw(2) << h << "|";rep(w, min(sz(a[h]),12ll)) { if (abs(a[h][w]) == linf) ss << "   e" << ""; else ss << std::right << std::setw(4) << a[h][w]; }ss << "" << endl;}return ss.str();}
...
public:
    ...
    //偏角でソート出来る
    bool operator<( D& v)  {        int o = ort(), vo = v.ort();        if (o != vo) return o < vo;        return cross(v) > 0;    }
    bool operator>( D &v)  {        int o = ort(), vo = v.ort();        if (o != vo) return o > vo;        return cross(v) < 0;    }
...
bool eq(D a, D b){ return dis(a,b) <= eps;}

//線分
struct E{
    D f,t;
    E(D f=D(),D t=D()):f(f),t(t){}
};
...
struct R{
    ...
    R():o(D(0,0)),r(0){}
    R(D o, dou r):o(o),r(r){}
    bool operator==(R b){return b.o == o && b.r == r;}
};

istream& operator>>(istream& ist, D& v) {ist >> v.x >> v.y; return ist;}
ostream& operator<<(ostream& os,  D& v) {os<<"("<<v.x<<","<<v.y<<")"; return os;}
ostream& operator<<(ostream& os,  R& v) {os<<"("<<v.o<<","<<v.r<<")"; return os;}
D vect(dou theta, dou len, D a = D());
double arg(D b, D a = D());
//@基本計算 取得***********************************************

//外積
double cross( D p,  D q) { return p.x * q.y - p.y * q.x; }
//距
double dis( D &p,  D &q){return std::sqrt(pow((p.x-q.x).v,2.) +pow((p.y-q.y).v,2.)) ;}
double dis2( D &p,  D &q){return (pow((p.x-q.x).v,2.) +pow((p.y-q.y).v, 2.)) ;}

//@判定*********************************************************
//線分の交差判定 端点を認めない
bool kousa_mid(D fa,D ta,D fb,D tb) {    dou ax = fa.x;    dou ay = fa.y;    dou bx = ta.x;    dou by = ta.y;    dou cx = fb.x;    dou cy = fb.y;    dou dx = tb.x;    dou dy = tb.y;  dou ta1 = (cx - dx) * (ay - cy) + (cy - dy) * (cx - ax);  dou tb1 = (cx - dx) * (by - cy) + (cy - dy) * (cx - bx);  dou tc1 = (ax - bx) * (cy - ay) + (ay - by) * (ax - cx);  dou td1 = (ax - bx) * (dy - ay) + (ay - by) * (ax - dx);  return tc1 * td1 < 0 && ta1 * tb1 < 0;}
bool kousa_mid(E f,E t){return kousa_mid(f.f,f.t,t.f,t.t);}
//端点を認める
bool kousa_hasi(D fa,D ta,D fb,D tb) {    dou ax = fa.x;    dou ay = fa.y;    dou bx = ta.x;    dou by = ta.y;    dou cx = fb.x;    dou cy = fb.y;    dou dx = tb.x;    dou dy = tb.y;  dou ta1 = (cx - dx) * (ay - cy) + (cy - dy) * (cx - ax);  dou tb1 = (cx - dx) * (by - cy) + (cy - dy) * (cx - bx);  dou tc1 = (ax - bx) * (cy - ay) + (ay - by) * (ax - cx);  dou td1 = (ax - bx) * (dy - ay) + (ay - by) * (ax - dx);   return tc1 * td1 <= 0 && ta1 * tb1 <= 0;}
bool kousa_hasi(E f,E t){return kousa_hasi(f.f,f.t,t.f,t.t);}
//交差する点を0~2個返す 同じ円が渡された場合、空を返す
dots kousa(R a, R b) {    if(a==b)return dots();auto d = dis(a.o, b.o);    if (d > a.r + b.r)return dots();    if (eq(d, a.r + b.r))return dots{(a.o + b.o) / 2};    dou u = acos(((a.r * a.r) + (d * d) - (b.r * b.r)) / (2 * a.r * d));    dou t = arg(b.o - a.o);    return dots{vect(t + u, a.r, a.o), vect(t - u, b.r, a.o)};}


//@角度系 中心とするものの引数は右に*****************************

//偏角 [-pi,pi] aを中心としたbの偏角
double arg(D b, D a /*= D()*/) {b -= a;    auto d= atan2(b.y, b.x);    return d;}

//aを中心にbを何シータ 反時計回りに回転させたらcになるかを返す
//[0, 2pi)
double ang(D b,D c,D a=D(0,0)){    c-=a;    b-=a;    int ob = b.ort();    int oc = c.ort();    if(!ob || !oc){        cerr<<"同じ点は渡せない"<<endl;        re();    }    double ret=0;    if(ob < oc){        ret += (oc-ob-1) * PI/2;        ret += arg(b.ort_bound_next()) - arg(b);        /*prevが-piではなくpiになる       */ if(oc==3){            ret += arg(c)-(-PI);        }else{            ret += arg(c)- arg(c.ort_bound_prev());        }    }else if(ob>oc){        return PI*2 - ang(c,b,D(0,0));    }else{        if(b > c){            return PI*2 - ang(c,b,D(0,0));        }else{            ret = arg(c) - arg(b);        }    }    return ret;}
//角度 aを中心とする最小角度の絶対値
double uang(D b, D c, D a = D(0, 0)) {            dou ret = ang(b, c, a);            return min(ret, 2 * PI - ret);        }
//@余弦定理*******************************************************
//todo

//@変形***********************************************************
//bを回転 aを中心に反時計回り
D rot(D b,dou theta,D a=D(0,0)){    b-=a;    dou x=b.x,y=b.y;    return D(cos(theta) * x -sin(theta) * y, sin(theta) * x + cos(theta)*y)+a;}
D mid(D& a,D& b){return (a+b)/2;}
D change_norm(D b, double want_norm, D a = D(0, 0)) {    b -= a;    b /= b.norm();    b *= want_norm;    return b + a;}
//偏角と長さから座標を返す[aを中心とする]
D vect(dou theta, dou len, D a /*= D()*/){return D(cos(theta) * len, sin(theta) * len) + a;}

//@アルゴリズム***************************************************
// 一直線上の3点を含めない
// 全ベクトルが同じ方向でどれを足し合わせるかを考えるときに一直線にすると爆発するhttps://atcoder.jp/contests/abc139/tasks/abc139_f F-Engines
//反時計回り
vector<D> convexhull(vector<D> &ps) {    int n = (int) ps.size();    if (n <= 2) { return ps; }    vector<D> res(2 * n);    std::sort(ps.begin(), ps.end(), [&](D &l, D &r) { if (l.x != r.x)return l.x < r.x; else return l.y < r.y; });    int k = 0;    for (int i = 0; i < n; ++i) {        if (k >= 2) {            while (cross(res[k - 1] - res[k - 2], ps[i] - res[k - 2]) <= eps) {                --k;                if (k < 2) break;            }        }        res[k] = ps[i];        ++k;    }    int t = k + 1;    for (int i = n - 2; i >= 0; --i) {        if (k >= t) {            while (cross(res[k - 1] - res[k - 2], ps[i] - res[k - 2]) <= eps) {                --k;                if (k < t) break;            }        }        res[k] = ps[i];        ++k;    }    res.resize(k - 1);    return res;}
//インデックスを返す
vi convexhull_i(vector<D> &ps) {    int n = (int) ps.size();    if (n <= 2) {        if (n == 1)return vi{0};else return vi{0, 1};}vi res(2 * n);vi ind = iota(0, n);    std::sort(ind.begin(), ind.end(), [&](int l, int r) {        if (ps[l].x != ps[r].x)return ps[l].x < ps[r].x;        else return ps[l].y < ps[r].y;    });    int k = 0;    for (int i = 0; i < n; ++i) {        if (k >= 2) {            while (cross(ps[res[k - 1]] - ps[res[k - 2]], ps[ind[i]] - ps[res[k - 2]]) <= eps) {                --k;                if (k < 2) break;            }        }        res[k] = ind[i];        ++k;    }    int t = k + 1;    for (int i = n - 2; i >= 0; --i) {        if (k >= t) {            while (cross(ps[res[k - 1]] - ps[res[k - 2]], ps[ind[i]] - ps[res[k - 2]]) <= eps) {                --k;                if (k < t) break;            }        }        res[k] = ind[i];        ++k;    }    res.resize(k - 1);    return res;}
// 一直線上の3点を含める
vector<D> convexhull_line(vector<D> &ps) {    int n = (int) ps.size();    if (n <= 2) { return ps; }    vector<D> res(2 * n);    std::sort(ps.begin(), ps.end(), [&](D &l, D &r) { if (l.x != r.x)return l.x < r.x; else return l.y < r.y; });    int k = 0;    for (int i = 0; i < n; ++i) {        if (k >= 2) {            while (cross(res[k - 1] - res[k - 2], ps[i] - res[k - 2]) < -eps) {                --k;                if (k < 2) break;            }        }        res[k] = ps[i];        ++k;    }    int t = k + 1;    for (int i = n - 2; i >= 0; --i) {        if (k >= t) {            while (cross(res[k - 1] - res[k - 2], ps[i] - res[k - 2]) < -eps) {                --k;                if (k < t) break;            }        }        res[k] = ps[i];        ++k;    }    res.resize(k - 1);    return res;}
vi convexhull_line_i(vector<D> &ps) {    int n = (int) ps.size();    if (n <= 2) {        if (n == 1)return vi{0};        else return vi{0, 1};    }    vi res(2 * n);    vi ind = iota(0, n);    std::sort(ind.begin(), ind.end(), [&](int l, int r) {        if (ps[l].x != ps[r].x)return ps[l].x < ps[r].x;        else return ps[l].y < ps[r].y;    });    int k = 0;    for (int i = 0; i < n; ++i) {        if (k >= 2) {            while (cross(ps[res[k - 1]] - ps[res[k - 2]], ps[ind[i]] - ps[res[k - 2]]) < -eps) {                --k;                if (k < 2) break;            }        }        res[k] = ind[i];        ++k;    }    int t = k + 1;    for (int i = n - 2; i >= 0; --i) {        if (k >= t) {            while (cross(ps[res[k - 1]] - ps[res[k - 2]], ps[ind[i]] - ps[res[k - 2]]) < -eps) {                --k;                if (k < t) break;            }        }        res[k] = ind[i];        ++k;    }    res.resize(k - 1);    return res;}/*@formatter:on*/
//D := dots
//R(D, len)
//dis, dis2(距離の2乗)
//kousa
...
void solve() {
    ...
    auto f = [&](dou time) {
        ...
        fora(d, koho) {                                                                                                          /*@formatter:off*/fora_init(d,koho );/*@formatter:on*/
            int cou = 0;
            rep(i, N) {
                auto t = ds[i];
                if (dis(d, t) * C[i] <= time)cou++;
            }
