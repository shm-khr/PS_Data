...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
        if (dist[0][i] == dist[1][i] && dist[0][i] + dist[1][i] == dist[0][end_pos])
            ans = (0LL + ans + mod - 1LL * dp[0][i] * dp[1][i] % mod) % mod;
    for (int i = 1; i <= m; i++)
        for (int rd = 0; rd < 2; rd++)
        {
            if (dist[0][xi[i]] + wi[i] == dist[0][yi[i]])
            {
                // included;
                if (dist[0][xi[i]] < 1.0 * dist[0][end_pos] / 2.0 && dist[1][yi[i]] < 1.0 * dist[0][end_pos] / 2.0)
                    ans = (0LL + ans + mod - 1LL * dp[0][xi[i]] * dp[0][xi[i]] % mod * dp[1][yi[i]] % mod * dp[1][yi[i]] % mod) % mod;
            }
            swap(xi[i], yi[i]);
        }
    printf("%d\n", ans);
